...
class DpHbpContinuum : public PlasticContinuum
{
  protected:
    Real yield_stress_;
    Real consistency_index_;
  ...
  public:
    explicit DpHbpContinuum(Real rho0, Real c0, Real youngs_modulus, Real poisson_ratio,
                            Real friction_angle, Real cohesion,
                            Real yield_stress, Real consistency_index, Real flow_index,
                            Real regularization_m, Real min_shear_rate = 1.0e-6,
                            Real max_shear_rate = 1.0e6)
        : PlasticContinuum(rho0, c0, youngs_modulus, poisson_ratio, friction_angle, cohesion),
          yield_stress_(yield_stress), consistency_index_(consistency_index), flow_index_(flow_index),
          regularization_m_(regularization_m), min_shear_rate_(min_shear_rate), max_shear_rate_(max_shear_rate)
    {
        material_type_name_ = "DpHbpContinuum";
    };
    virtual ~DpHbpContinuum() {};
    ...
    Mat3d HbpConstitutiveRelation(Mat3d &velocity_gradient, Mat3d &stress_tensor);
};
